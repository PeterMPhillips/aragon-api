...
class AppProxy {
  ...
  guiStyle() {
    return this.rpc.sendAndObserveResponses('gui_style', ['observe']).pipe((0, _operators.pluck)('result'));
  }
  /**
   * Request a new path.
   *
  ...
  triggers() {
    return this.rpc.sendAndObserveResponses('trigger', ['observe']).pipe((0, _operators.pluck)('result'));
  }
  /**
  ...
  datastore(method, ...params) {
    return this.rpc.sendAndObserveResponse('datastore', [method, ...params]).pipe((0, _operators.pluck)('result'));
  }
  /**
  ...
  store(reducer, {
    ...
    const wrappedReducer = ({
      state
    }, event) => (0, _rxjs.from)( // Ensure a promise is returned even if the reducer returns an array or throws
    new Promise(resolve => resolve(reducer(state, event)))).pipe((0, _operators.map)(state => ({
      state,
      lastEvent: event
    ...
    const store$ = (0, _rxjs.forkJoin)(cacheValue$, initState$, latestBlock$).pipe((0, _operators.switchMap)(([cacheValue, initState, latestBlock]) => {
      ...
      const events$ = (0, _rxjs.concat)( // Past events
      ...
      (0, _rxjs.defer)(() => {
        ...
        const currentEvents$ = getCurrentEvents(pastEventsToBlock + 1); // Observable which emits an web3.js event-like object with the address of the active account.
        ...
        return (0, _rxjs.merge)(currentEvents$, accounts$);
      }));
      const eventsWithTriggers$ = (0, _rxjs.merge)(events$, this.triggers());
      return eventsWithTriggers$.pipe( // Reduce
      (0, _operators.mergeScan)(wrappedReducer, {
        state: initialStoreState
      }, 1), // Cache commited state when past events have synced
      (0, _operators.tap)(async ({
        state,
        lastEvent = {}
      }) => {
        if (lastEvent.event === events.SYNC_STATUS_SYNCED) {
          (0, _utils.debug)("- store - caching state (block ".concat(pastEventsToBlock, "):"), state);
          const cacheState = {
