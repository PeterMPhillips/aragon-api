...
exports.debug = debug;
exports.getIconBySize = getIconBySize;
...
function getIconBySize(icons, size = -1) {
  // Collect the sizes and sort them
  const sizes = icons.map((icon, i) => {
    const width = parseInt(icon.sizes.split('x')[1], 10);
    return [i, isNaN(width) ? -1 : width];
  }).filter(size => size[1] !== -1).sort((a, b) => a[1] - b[1]); // No valid size found
  ...
  const iconIndex = (sizes.find(iconSize => iconSize[1] >= size) || sizes[sizes.length - 1])[0];
